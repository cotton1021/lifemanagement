...
if ($_POST['action'] === 'update') {
	...
	foreach ($todo_select as $id) {
		// `todo_list_roop` と `todo_list` を取得
		...
		if ($task) {
			...
			if ($task['todo_list_roop'] == 1) {
				...
				// 同じ `todo_list` 内で最大の `todo_priority_num` を取得
				$stmt = $dbh->prepare("SELECT MAX(todo_priority_num) 
					FROM todo_main 
					WHERE todo_list = ? AND todo_complete = 0
				");
				$stmt->execute([$todo_list]);
				$max_priority = $stmt->fetchColumn();

				// `todo_priority_num` を決定（最大値 +1、または 1）
				$todo_priority_num = ($max_priority !== null) ? $max_priority + 1 : 1;

				$stmt = $dbh->prepare("INSERT INTO todo_main 
						(todo_name, todo_list, todo_roop, todo_medium, todo_place, todo_priority, todo_priority_num, todo_url, todo_start,  todo_note, todo_create_date)
					SELECT 
						todo_name, todo_list, todo_roop, todo_medium, todo_place, todo_priority, ?, 
						todo_url, CURDATE(), todo_note, CURDATE()
					FROM todo_main 
					WHERE todo_id = ?
				");
				$stmt->execute([$todo_priority_num, $id]);
			}
		}
	}
