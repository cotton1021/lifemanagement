...
<body id="item" class="item_main">
	...
	<div class="link_area">
		...
		<?php
		} elseif ($_GET['next'] != "" && $prev_year >= $this_year) {
		?>
			<a class="prev" href="./mypage.php?year=<?php echo $prev_year ?>&next=1">過去の<br class="sp">購入履歴</a>
		<?php
		} else {
		?>
			<span>　　　　　　　　</span>
		...
		if ($_GET['prev'] == "") {
		?>
			<a class="next" href="./mypage.php?year=<?php echo $next_year ?>&next=1">
				<?php
				if ($_GET['year'] == "") {
					echo date('n/j', strtotime($next_month_end));
				}
				?>
				以降の<br class="sp">購入予定</a>
		<?php
		} elseif ($_GET['prev'] != "" && $next_year < $this_year) {
		?>
			<a class="next" href="./mypage.php?year=<?php echo $next_year ?>&prev=1">
				<?php
				if ($_GET['year'] == "") {
					echo date('n/j', strtotime($next_month_end));
				}
	...
	<form method="post" action="../../inc/do/update_items.php">
		...
		if ($_GET['year'] == '') {
			...
			$disp_num = 2;
			$disp_title = "<h2>【当月】" . date('Y', strtotime($month_start)) . "年" . date('n', strtotime($month_start)) . "月の購入品</h2>";
		} elseif ($_GET['year'] && $_GET['prev'] != "") {
			$date_setting_start = $archive_month_start . " 00:00:00";
			$date_setting_end = $archive_month_end . " 23:59:59";
			$disp_num = 12;
			$disp_title = "<h2>" . date('Y', strtotime($archive_month_start)) . "年" . date('n', strtotime($archive_month_start)) . "月の購入品</h2>";
		} elseif ($_GET['year'] && $_GET['next'] != "") {
			$date_setting_start = $future_month_start . " 00:00:00";
			$date_setting_end = $future_month_end . " 23:59:59";
			$disp_num = 12;
			$disp_title = "<h2>" . date('Y', strtotime($future_month_start)) . "年" . date('n', strtotime($future_month_start)) . "月の購入品</h2>";
		}
		...
		<?php
			if ($_GET['year'] == '') {
				...
				$disp_title = "<h2>【翌月】" . date('Y', strtotime($next_month_start)) . "年" . date('n', strtotime($next_month_start)) . "月の購入品</h2>";
