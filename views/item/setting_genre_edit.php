...
?>
	<section>
		<h2>ジャンル編集</h2>
		<form method="POST" id="form" action="../../inc/do/add_genre.php">
			<input type="hidden" name="com_genre_id" value="<?php echo $rec['com_genre_id']?>">
			<table>
				<tr>
					<th>タイトル</th>
					<td><input type="text" name="com_genre" value="<?php echo $rec['com_genre']?>"></td>
				</tr>
				<tr>
					<th>並び順</th>
					<td><input type="text" name="com_genre_order" value="<?php echo $rec['com_genre_order']?>"></td>
				</tr>
				<tr>
					<th>削除</th>
  ...
  $('.price_check').on('click', function() {
	...
	$('#update').on('click', function() {
		if($("#item_delete_flag").prop('checked')){
			if(confirm('<?php echo $rec['com_genre']?>を削除しますか？')){
